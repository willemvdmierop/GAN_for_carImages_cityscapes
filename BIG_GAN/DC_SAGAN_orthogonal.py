...
# Orthogonal param. regularization. Source: https://github.com/ajbrock/BigGAN-PyTorch/blob/master/utils.py #
...
dirname = 'model_DC_SAGAN_' + 'orthogonal_' 'batch_' + str(batch_size) + "_wd" + w_decay_str + "_lr" + lrate_str
...
if not os.path.exists(wd + '/gen_images_crpd_ortho_DC_SAGAN'):
	os.mkdir(wd + '/gen_images_crpd_ortho_DC_SAGAN')
...
if os.path.exists(os.path.join(folder_name, 'checkpoint.pth')):
	...
	try:
		...
		weights_g = torch.load(
			os.path.join(folder_name, "gen_crpd_DC_SAGAN_" + 'orthogonal_' + "batch_" + batch_size_str + "_wd" + w_decay_str +
...
# ================================================= Training ================================================== #
# loss function
tb = SummaryWriter(comment="ortho_DC_SAGAN_batch" + str(batch_size) + "_wd" + w_decay_str + "_lr" + lrate_str)
loss = BCELoss()
loss = loss.to(device)
# main train loop
...
for e in range(epochs, num_epochs):
	...
	if e % 50 == 0:
		...
		torch.save(g.state_dict(), os.path.join(folder_name,
												"gen_crpd_DC_SAGAN_" + 'orthogonal_' + "batch_" + batch_size_str + "_wd" + w_decay_str + "_lr" + lrate_str + "_e" + str(
													e) + ".pth"))
		torch.save(d.state_dict(), os.path.join(folder_name,
												"dis_crpd_DC_SAGAN_" + 'orthogonal_' + "batch_" + batch_size_str + "_wd" + w_decay_str + "_lr" + lrate_str + "_e" + str(
													e) + ".pth"))
		...
		for i in range(5):
			if not os.path.exists(wd + "/gen_images_crpd_ortho_DC_SAGAN/" + "hallucinated_" + str(e)):
				os.mkdir(wd + "/gen_images_crpd_ortho_DC_SAGAN/" + "hallucinated_" + str(e))
			...
			plt.imshow(out)
			filename = wd + "/gen_images_crpd_ortho_DC_SAGAN/" + "hallucinated_" + str(e) + "/generated_" + str(i) + ".png"
